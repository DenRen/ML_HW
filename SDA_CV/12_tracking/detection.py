...
def image2tensor(image):
    ...
    image -= IMAGENET_MEAN[..., -1]  # center with respect to imagenet means
...
def extract_detections(frame, min_confidence=0.6, labels=None):
    ...
    if len(results) > 0:
        ...
        X, Y = results[:, 1::2], results[:, 2::2]
    
        X = np.clip(X * frame.shape[1], 0, frame.shape[1]-1).tolist()
        Y = np.clip(Y * frame.shape[0], 0, frame.shape[0]-1).tolist()
        
        results[:, 1::2], results[:, 2::2] = X, Y
...
def main():
    ...
    frame = Image.open("tests/00_unittest_detection_input/new_york.jpg")
